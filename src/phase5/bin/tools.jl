...
function write_tour(filename::String, tour::Vector{Int64}, cost::Float64)
...
"""Read a tour file in the TSP format."""
function read_tour(tour_filename::String)
    tour = Int[]
...
"""Reorder a tour that doesn't start with node 0."""
function reorder_tour(tour::Vector{Int64})
    return [tour[argmin(tour):end]; tour[1:argmin(tour)-1]]
end
